...
<body>
    ...
    <div class="header">
        @if($whiteLabel ?? false)
            @if(!empty($companyLogoBase64))
                <img src="{{ $companyLogoBase64 }}" alt="" style="max-height: 60px; max-width: 300px; margin: 0 auto 10px; display: block;">
            @endif
            <h1>{{ !empty($companyName) ? $companyName : 'Accessibility Report' }}</h1>
        @else
            <h1>Access Report Card Accessibility Report</h1>
        @endif
    ...
    <div class="footer">
        @if($whiteLabel ?? false)
            <p>{{ !empty($companyName) ? $companyName.' — ' : '' }}{{ $scan->url }}</p>
        @else
            <p>Generated by <strong>Access Report Card</strong> — {{ $scan->url }}</p>
        @endif
